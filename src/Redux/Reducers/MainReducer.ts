...
export default (state = INITIAL_STATE, action: MainActions): MainReducer => {
    ...
    switch (action.type) {
        case GET_POSTS_SUCCESS:
            return { ...state, getPostsSuccess: true, data: { ...state.data, posts: action.payload }};
        case GET_POSTS_FAILED:
            return { ...state, getPostsFail: true, error: action.payload };

        case GET_ALBUMS_SUCCESS:
            return { ...state, getAlbumsSuccess: true, data: { ...state.data, albums: action.payload }};
        case GET_ALBUMS_FAILED:
            return { ...state, getAlbumsFail: true, error: action.payload };

        case GET_PHOTOS_SUCCESS:
            return { ...state, getPhotosSuccess: true, data: { ...state.data, photos: action.payload }};
