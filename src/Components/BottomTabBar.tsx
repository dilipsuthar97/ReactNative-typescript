...
const springValue1 = new Animated.Value(0.9);
const springValue2 = new Animated.Value(0.8);
const springValue3 = new Animated.Value(0.8);
const springValue4 = new Animated.Value(0.8);
		...
		>
			{state.routes.map((route, index) => {
				...
				const spring = (val: number) => {
					...
					if (val == 0) {
						Animated.spring(springValue1, {
							toValue: 0.9,
							friction: 1,
							useNativeDriver: true,
						});
					} else if (val == 1) {
						Animated.spring(springValue2, {
							toValue: 0.9,
							friction: 1,
							useNativeDriver: true,
						}).start();
					} else if (val == 2) {
						Animated.spring(springValue3, {
							toValue: 0.9,
							friction: 1,
							useNativeDriver: true,
						}).start();
					} else if (val == 3) {
						Animated.spring(springValue4, {
							toValue: 0.9,
							friction: 1,
							useNativeDriver: true,
						}).start();
				...
				return (
					...
					>
						{index == 0 && (
							<Animated.Image
								source={Images.IC_POSTS}
								style={[
									{
										height: isFocused ? Scale(32) : Scale(30),
										width: isFocused ? Scale(32) : Scale(30),
										tintColor: isFocused ? Colors.PRIMARY_COLOR : Colors.GRAY,
									},
									springValue1 ? { transform: [{ scale: springValue1 }] } : {},
								]}
								resizeMode={'contain'}
							/>
						)}
						{index == 1 && (
							<Animated.Image
								...
								style={[
									...
									springValue2 ? { transform: [{ scale: springValue2 }] } : {},
								]}
								resizeMode={'contain'}
							/>
						)}
						{index == 2 && (
							<Animated.Image
								...
								style={[
									...
									springValue3 ? { transform: [{ scale: springValue3 }] } : {},
								]}
								resizeMode={'contain'}
							/>
						)}
